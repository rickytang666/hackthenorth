...
@triton.jit
def _norm_rope_heads(X, W, Y, cos, sin, sign, base, out_base, HEADS: tl.constexpr,
                     BLOCK_H: tl.constexpr, D: tl.constexpr, EPS: tl.constexpr):
    dim = tl.arange(0, D)
    other = (dim + D // 2) % D
    weight = tl.load(W + dim).to(tl.float32)
    other_weight = tl.load(W + other).to(tl.float32)
    dtype: tl.constexpr = Y.dtype.element_ty
    for start in range(0, HEADS, BLOCK_H):
        head = start + tl.arange(0, BLOCK_H)
        x = tl.load(X + base + head[:, None] * D + dim[None, :]).to(tl.float32)
        rotated = tl.load(X + base + head[:, None] * D + other[None, :]).to(tl.float32)
        inv = tl.rsqrt(tl.sum(x * x, 1) / D + EPS)
        normed = ((x * inv[:, None]).to(dtype).to(tl.float32)
                  * weight[None, :]).to(dtype).to(tl.float32)
        rotated = ((rotated * inv[:, None]).to(dtype).to(tl.float32)
                   * other_weight[None, :]).to(dtype).to(tl.float32)
        rotated = tl.where(sign[None, :], -rotated, rotated)
        left = (normed * cos[None, :]).to(dtype).to(tl.float32)
        right = (rotated * sin[None, :]).to(dtype).to(tl.float32)
        tl.store(Y + out_base + head[:, None] * D + dim[None, :], left + right)


@triton.jit
def _norm_rope_tile(Q, K, WQ, WK, OQ, OK, COS, SIN,
                    TOKENS: tl.constexpr, Q_HEADS: tl.constexpr, K_HEADS: tl.constexpr,
    ...
    sin = tl.load(SIN + token * D + dim).to(tl.float32)
    sign = dim < D // 2
    _norm_rope_heads(Q, WQ, OQ, cos, sin, sign,
                     batch * Q_BATCH + token * Q_STRIDE,
                     (batch * TOKENS + token) * Q_HEADS * D,
                     Q_HEADS, BLOCK_H, D, Q_EPS)
    _norm_rope_heads(K, WK, OK, cos, sin, sign,
                     batch * K_BATCH + token * K_STRIDE,
                     (batch * TOKENS + token) * K_HEADS * D,
                     K_HEADS, K_HEADS, D, K_EPS)


def norm_rope(q, k, q_norm, k_norm, cos, sin, tile=True):
    ...
    if tile:
        ...
        _norm_rope_tile[(b * tokens,)](
            q, k, q_norm.weight, k_norm.weight, out_q, out_k, cos, sin,
            tokens, q_heads, k_heads, 16, dim,
            q_norm.variance_epsilon, k_norm.variance_epsilon,
