"""Before/after worktree decode-attention fusion and calibration, fixed Flash prefill."""
import gc
import json
import statistics
import subprocess
...
import projections as BaselineProjections
from prompts import corpora, prompts, emit


def run(engine, inputs, length):
    sys.modules["projections"] = engine.calibration_module
...
def main(mode):
    ...
    if mode == "production":
        ...
        import projections as CandidateProjections
        mode = "full"
    ...
    baseline.calibration_module = BaselineProjections
    candidate.calibration_module = CandidateProjections
    ...
    for shape in cases:
        ...
        BaselineProjections._choices.clear()
        CandidateProjections._choices.clear()
