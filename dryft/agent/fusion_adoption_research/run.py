"""Isolated H100 validation of the tiled prefill normalization/RoPE kernel."""
import json
from pathlib import Path

import modal

HERE = Path(__file__).resolve().parent if modal.is_local() else Path("/research/overlap")
ROOT = HERE.parents[1] if modal.is_local() else Path("/research/production")
app = modal.App("dryft-decode-fusion-adoption")
...
image = (
    ...
    .add_local_dir("/tmp/dryft-fusion-adoption-baseline/engine", "/research/engine")
    .add_local_dir(ROOT / "agent/prefill_research", "/research/prefill", ignore=["**/__pycache__/**"])
    .add_local_dir(HERE, "/research/overlap", ignore=["**/__pycache__/**"])
...
@app.function(image=image, gpu="H100!", cpu=8, memory=32768, timeout=1800,
...
def experiment(mode: str):
    ...
    process = subprocess.Popen([sys.executable, "-u", "/research/overlap/bench.py", mode],
...
def main(mode: str = "production"):
    ...
    target = HERE.parents[1] / "experiments" / "25-worktree-attention-fusion" / f"{mode}.json"
