"""The measured-fastest projection tile with optional fused norm and residual.

A faithful copy of Triton 3.1's ops.matmul kernel (the in-graph winner for
qkv/o/down at decode rows), specialized to SPLIT_K=1 and BF16, with two
hooks that cut a kernel and a tensor round trip each:

- NORM: RMS-normalize the activation rows in-program (Qwen3RMSNorm's exact
  cast order: normalize in FP32, round to BF16, multiply by the BF16 gain),
  so the separate norm kernel and its normalized-tensor write+read vanish.
- RESIDUAL: round the FP32 accumulator to BF16, add the BF16 residual, and
  round again at the store - the native boundary of projection-then-add -
  so the separate elementwise add and its output round trip vanish.
...
@triton.jit
def _fused_tile(A, B, C, GAIN, RES, M, N, K,
    ...
    if RESIDUAL:
        ...
        tl.store(Cptr, (projected + residual).to(tl.bfloat16), mask=mask)
    else:
        tl.store(Cptr, acc.to(C.dtype.element_ty), mask=mask)


def tile_projection(x, weight, rows, tile, gain=None, eps=0.0, residual=None):
    """Launch the fused tile exactly as Projection._tile launches upstream.

    x is (rows, K) BF16 contiguous; weight is (N, K) in row or column
    storage (strides carry the layout). Passing `gain` fuses the RMS-norm
    prologue; passing `residual` (rows, N) fuses the add epilogue.
    """
    n, k = weight.shape
    out = torch.empty((rows, n), device=x.device, dtype=x.dtype)
    bn, bk, warps, stages = tile
    _fused_tile[(triton.cdiv(rows, 16) * triton.cdiv(n, bn),)](
        x, weight, out,
        x if gain is None else gain,
        out if residual is None else residual,
        rows, n, k,
        x.stride(0), x.stride(1), weight.stride(1), weight.stride(0),
        out.stride(0), out.stride(1),
        eps, gain is not None, residual is not None,
        16, bn, bk, 8, k % bk == 0,
